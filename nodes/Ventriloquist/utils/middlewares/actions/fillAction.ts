...
export async function executeFillAction(
	...
	try {
		...
		try {
			...
			if (!success) {
				throw new Error(
					`Failed to fill form field: ${selector} (type: ${fieldType})`,
				);
			}
			...
			if (pressEnter) {
				...
				if (
					...
					try {
						...
						logger.info(
							`Using submitFormAndWaitForNavigation utility with ${pressEnter ? "Enter key" : "direct action"}`,
						);

						const navigationResult = await submitFormAndWaitForNavigation(
							sessionId,
							{
								timeout: 30000,
						...
						if (navigationResult.success) {
							...
							try {
								...
								contextDestroyed =
									(stateError as Error).message.includes(
				...
				// Handle fixed time waiting
					...
					try {
						...
						if (
							(contextError as Error).message.includes(
								"context was destroyed",
							) ||
				...
				if (waitAfterAction === "waitForSelector" && waitSelector) {
					try {
						...
						if (
							(selectorError as Error).message.includes(
								"context was destroyed",
							) ||
			...
			if (
				(processError as Error).message.includes("context was destroyed") ||
